      ...
      <nav className="max-w-[1200px] mx-auto flex items-center justify-between gap-6 px-6 py-4">
              ...
              <li key={index}>
                <Link
                  ...
                  className={`[font-family:'Open_Sans',Helvetica] text-sm tracking-[0] leading-[22px] whitespace-nowrap px-4 py-2.5 rounded-lg transition-all duration-500 ease-in-out relative overflow-hidden group ${item.label === activePage
                      ? "font-semibold text-white bg-[#8B2E34] shadow-md"
                      : "font-medium text-[#1A1A1A] hover:text-[#A23C43] hover:bg-gradient-to-r hover:from-[#f6f1e7] hover:to-white"}`}
                >
                  {item.label === activePage && (
                    <span className="absolute inset-0 bg-gradient-to-r from-[#8B2E34] to-[#A23C43] -z-10"></span>
                  )}
          ...
          <div className="flex items-center gap-2 pl-4 border-l border-[#e0d7c7]">
            {socialMediaIcons.map((icon, index) => (
              <a
                ...
                className="group"
              >
                <div className="w-10 h-10 rounded-lg p-0 bg-gradient-to-br from-white to-[#f6f1e7] border border-[#e7dece] hover:border-[#8B2E34] hover:shadow-md transition-all duration-500 ease-in-out flex items-center justify-center group-hover:scale-110">
        ...
        <div className="md:hidden flex items-center">
          <button
            ...
            className="w-11 h-11 rounded-lg flex items-center justify-center border-2 border-[#e0d7c7] hover:border-[#8B2E34] hover:bg-[#f6f1e7] transition-all duration-500 ease-in-out active:scale-95"
          >
        ...
        <aside className={`fixed top-20 right-4 z-50 w-[340px] bg-gradient-to-b from-white to-[#f6f1e7] rounded-2xl shadow-2xl border border-[#e0d7c7] transform transition-all duration-500 ease-out ${openMobile ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 -translate-y-4 scale-95 pointer-events-none'}`} role="dialog" aria-modal="true">
          <div className="relative">
            <button
              ...
              className="absolute right-4 top-4 w-9 h-9 rounded-lg flex items-center justify-center border-2 border-[#e0d7c7] bg-white hover:border-[#8B2E34] hover:bg-[#f6f1e7] transition-all duration-500 ease-in-out active:scale-95"
            >
                    ...
                    <li key={idx}>
                      <Link
                        ...
                        className={`block px-4 py-3 rounded-lg text-base [font-family:'Open_Sans',Helvetica] font-medium transition-all duration-500 ease-in-out ${item.label === activePage
                          ? 'text-white bg-gradient-to-r from-[#8B2E34] to-[#A23C43] shadow-md'
                          : 'text-[#1A1A1A] hover:text-[#A23C43] hover:bg-white hover:shadow-sm'}`}
                      >
                        {item.label}
                      </Link>
                    </li>
              ...
              <div className="flex items-center gap-3">
                {socialMediaIcons.map((icon, index) => (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-10 h-10 rounded-lg bg-white border border-[#e0d7c7] hover:border-[#8B2E34] hover:shadow-md flex items-center justify-center transition-all duration-500 ease-in-out hover:scale-110"
                  >
